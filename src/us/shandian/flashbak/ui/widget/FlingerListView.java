...
public class FlingerListView extends ListView {
    ...
    private void init() {
        ...
        this.mFlingThreshold = convertDpToPixel(MIN_FLINGER_THRESHOLD);
    }
				...
				if (this.mFlingingView != null) {
					...
					if (!this.mScrolling) {
						if (ev.getX() - this.mCurrentX != 0) {
							...
							// Started
							if (!this.mFlingingStarted && Math.abs(this.mTranslationX) > this.mFlingThreshold) {
								// Flinging starting
